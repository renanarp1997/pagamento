    ...
    <AppShell>
      <section className="flex flex-col gap-5">
        <header className="rounded-3xl border border-slate-200 bg-white/95 p-5 shadow-soft dark:border-slate-800 dark:bg-slate-900/95 sm:p-7">
          <div className="flex flex-wrap items-end justify-between gap-4">
            ...
            <div className="flex gap-2">
              <select aria-label="Ano do resumo" value={year} onChange={(event) => setYear(Number(event.target.value))} className="h-12 rounded-xl border border-slate-200 bg-white px-4 font-black dark:border-slate-700 dark:bg-slate-950">{(years.length ? years : [year]).map((item) => <option key={item}>{item}</option>)}</select>
              <Link href="/" className="inline-flex h-12 items-center rounded-xl bg-slate-950 px-4 font-black text-white dark:bg-white dark:text-slate-950">Voltar</Link>
            </div>
          </div>
        </header>
        ...
        <section className="rounded-3xl border border-slate-200 bg-white/95 p-5 shadow-soft dark:border-slate-800 dark:bg-slate-900/95 sm:p-7">
          <div className="grid h-64 grid-cols-12 items-end gap-2" role="img" aria-label={`Gráfico de pagamentos de ${year}`}>
