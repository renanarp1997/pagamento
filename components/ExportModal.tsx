...
import type { MonthData, PeriodSummary } from "@/types/payment";
...
export function ExportModal({ year, month, data, isOpen, onClose }: ExportModalProps) {
  const reportRef = useRef<HTMLDivElement>(null);
  const [exporting, setExporting] = useState<string | null>(null);
  const [toast, setToast] = useState<{ type: "success" | "error"; message: string } | null>(null);
  const report = useMemo(() => buildExportReport(year, month, data), [year, month, data]);
  ...
  async function exportPdf() {
    ...
    try {
      ...
      pdf.setFontSize(22);
      pdf.text("Pagamento Quinzenal", 14, 16);
      pdf.setFont("helvetica", "normal");
      pdf.setFontSize(12);
      pdf.text(`${report.monthName} • ${report.year}`, 14, 25);
      pdf.setTextColor(15, 23, 42);

      autoTable(pdf, {
        startY: 44,
        head: [["Período", "Trabalhados", "Inteiros", "Meios", "Folgas", "Total"]],
        body: [
          ["Primeira quinzena", `${report.first.workedDays}/${report.first.totalDays}`, report.first.fullDays, report.first.halfDays, report.first.daysOff, formatCurrency(report.first.total)],
          ["Segunda quinzena", `${report.second.workedDays}/${report.second.totalDays}`, report.second.fullDays, report.second.halfDays, report.second.daysOff, formatCurrency(report.second.total)]
        ],
        theme: "grid",
      ...
      const tableEnd = (pdf as typeof pdf & { lastAutoTable?: { finalY: number } }).lastAutoTable?.finalY ?? 70;
      pdf.setFont("helvetica", "bold");
      pdf.setFontSize(13);
      pdf.text("Totais do mês", 14, tableEnd + 12);
      autoTable(pdf, {
        startY: tableEnd + 16,
        body: [
          ["Dias trabalhados", `${report.monthlyWorkedDays}/${report.monthlyTotalDays}`],
          ["Dias inteiros", `${report.first.fullDays + report.second.fullDays} × R$ ${FULL_DAY_VALUE}`],
          ["Meios períodos", `${report.first.halfDays + report.second.halfDays} × R$ ${HALF_DAY_VALUE}`],
          ["Dias de folga", String(report.first.daysOff + report.second.daysOff)],
      ...
      pdf.text(`Gerado em ${generatedAt}`, 14, 278);
      pdf.text("Generated by Quinzena Calculator", 196, 286, { align: "right" });
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          <div id="export-report-print" ref={reportRef} className="bg-white px-6 py-6 sm:px-8 sm:py-8">
            ...
            <PeriodReport title="Primeira Quinzena" summary={report.first} />
            <div className="my-8 h-px bg-gradient-to-r from-transparent via-slate-200 to-transparent" />
            <PeriodReport title="Segunda Quinzena" summary={report.second} />
...
function PeriodReport({ title, summary }: { title: string; summary: PeriodSummary }) {
  return (
    <section>
      ...
      <div className="mt-4 space-y-3">
        <CategoryCard
          ...
          calculation={`${summary.fullDays} × R$${FULL_DAY_VALUE}`}
        ...
        <CategoryCard
          ...
          calculation={`${summary.halfDays} × R$${HALF_DAY_VALUE}`}
